...
class BaseInstance3DBoxes:
    ...
    Note:
        ...
        상자는 하단 중앙에 있어서 원점의 상대 위치는 0.5 0.5 0 이 된다.
    ...
    def __init__(self, tensor, box_dim=7, with_yaw=True, origin=(0.5, 0.5, 0)):
        # tensor 형태라면 device에 저장
        if isinstance(tensor, torch.Tensor):
            device = tensor.device
        # 그렇지 않다면 cpu에 저장
        else:
            device = torch.device("cpu")
        # tensor를 float32 type의 pytorch로 변환
        ...
        if tensor.numel() == 0:
            ...
            # reshape을 사용하여 (0, box_dim) 형태로 변경 -> 새로운 텐서를 만들지 않으려고
            tensor = tensor.reshape((0, box_dim)).to(dtype=torch.float32, device=device)
        # ?? 2차원이고 size가 box_dim과 일치하지 않으면 AssertionError 발생
    ...
    def overlaps(cls, boxes1, boxes2, mode="iou"):
        """Calculate 3D overlaps of two boxes.     -> 3D 중첩 계산
        ...
        rows = len(boxes1)
        cols = len(boxes2)
        # 0일 경우 겹친 곳이 없으므로 빈 결과 반환
        if rows * cols == 0:
            return boxes1.tensor.new(rows, cols)

        # height overlap -> 높이부분에서 overlaps 계산
        ...
        # bev overlap -> BEV 평면에서의 overlap 계산
        ...
        # BEV 평면에서의 겹침을 계산한 뒤 결과를 overlap_bev에 저장

        # 3d overlaps -> 3D 공간에서의 overlaps 계산
    ...
    def new_box(self, data):
        ...
        # 클래스 타입 저장
        original_type = type(self)
        # 기존의 box_dim 및 with_yaw 속성을 가지고 현재 클래스의 타입으로 새로운 box 객체를 반환
