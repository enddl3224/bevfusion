...
class BEVFusion(Base3DFusionModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        # config camera backbone 써있는거 갖다 쓰면 됨
        ...
        # 여기부터 BEVFusion 이해하기 fuser를 기준으로 
        ...
        # heads의 값들에 쉽게 접근할 수 있게 pytorch의 ModuleDict() 사용
        self.heads = nn.ModuleDict()
        for name in heads:
            # heads 안에 데이터가 있다면 config 파일에 있는 값들로 채워준다.
    ...
    def forward_single(
        ...
        **kwargs,
        ...
        if self.training:
            ...
            # head = null,
            #      = [activation: relu,
            #         bboxcoder: [code_size: 10, ... , voxel_size: 0.075, 0.075],
            #         ... ,
            for type, head in self.heads.items():
                # map==null이라 대부분 object로 들어옴
                if type == "object":
                    # 예측값
                    # metas: img_metas
                    pred_dict = head(x, metas)    # truansfusion.py의 forward 함수 실행 -> TransFusionHead의 객체 head를 호출했기 때문?
                    # 정답과 예측값을 비교한 손실값
                    # gt_bboxes_3d: 정답 bbox
                    # gt_labels_3d: label
                    # pred_dict: 예상 bbox
        ...
        # training이 아닐때(test, val일 경우)
        else:
            outputs = [{} for _ in range(batch_size)]
            for type, head in self.heads.items():
                if type == "object":
                    pred_dict = head(x, metas)
                    # pred_dict을 사용하여 bbox 생성
