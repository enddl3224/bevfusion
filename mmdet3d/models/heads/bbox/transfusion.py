...
# config의 ln.1126에 TransFusionHead를 불러옴
...
class TransFusionHead(nn.Module):
    def __init__(
        ...
        bias="auto",
        # loss
        # registry로 인해 loss 함수 실행? -> 92번째 줄에 build_loss 함수를 통해 LOSSES.build를 실행하니까?
    ...
    def forward_single(self, inputs, img_inputs, metas):
        ...
        """
        # inputs: features = x (fuser, backbone, neck, camera:vtransform)정보가 담겨있을듯
        batch_size = inputs.shape[0]
        # cnn을 사용해서 lidar feature 찾는다. -> 3차원 정보로 예상
        ...
        # bev_pos = 2D grid
        # 2D 정보를 3D 정보가 담겨있는 곳과 동일한 위치에 저장
        ...
        # nuSenes 데이터 중에서 pedestrian과 traffic_cone 에 대해 테스트
        ...
        # 새로운 특성을 생성하고, 또 다른 위치 정보를 추출하여 이를 특성에 추가하는 작업
        ...
        for i in range(self.num_decoder_layers):
            ...
            # 이전의 center 값과 계산한 좌표값을 사용해서 중심값 보정
    ...
    def forward(self, feats, metas):
        ...
        """
        # feats: bevfusion.py의 head(x,meats) 에서 x에 해당
        if isinstance(feats, torch.Tensor):
            feats = [feats]
        # feats, [None], [metas]를 forwar_single 함수에 적용 -> 병렬처리
    ...
    def get_targets(self, gt_bboxes_3d, gt_labels_3d, preds_dict):
        ...
        # 예측값과 정답이 들어있는 res_tuple을 분할해서 반환
    ...
    def get_bboxes(self, preds_dicts, metas, img=None, rescale=False, for_roi=False):
        ...
        for layer_id, preds_dict in enumerate(preds_dicts):
            ...
            # object 안에 common_heads 안에 아래의 속성들이 있음
            ...
            # nuScense 데이터를 사용하므로 아래의 구문은 실행되지 않음
            ...
            # 객체검출 모델의 테스트 결과에 대한 후처리 다녜
            # 객체 필터링하고 NMS를 적용해서 겹치는 객체 제거하고 확률높은 객체 유지
            # temp에 common_heads 정보 들어있음
            ret_layer = []    # result 담을 리스트
            for i in range(batch_size):
                ...
                # nms_type가 None type이 아니라면 아래의 구문 실행
                # nms_type = null이므로 실행하지 않음
                if self.test_cfg["nms_type"] != None:
                    ...
                    for task in self.tasks:
                        ...
                        if task["radius"] > 0:
                            ...
                            # 원형 nms가 아닐 경우 (ex, 사각형 nms)
                            else:
                                # 사각형 nms를 위해 필요한 정보 준비
